...
class Mail:
    ...
    def set_message(self, header: str, message: str):
        self.message = MIMEText(message, 'plain', 'utf-8')
        self.message['Subject'] = Header(header, 'utf-8')

    def set_server(self, smtp: str, port: int):
        self.server = smtplib.SMTP_SSL(smtp, port)

    def connect_mail(self, username: str, password: str):
        self.username = username
        self.password = password

    def send_message(self, mail_receiver: str):
        self.server.login(self.username, self.password)
        self.server.sendmail(self.username, mail_receiver, self.message.as_string())
        self.server.quit()

    def spam_messages(self, mail_receiver: list):
        self.server.login(self.username, self.password)
    ...
    def bombing_message(self, mail_receiver: str, amount: int):
        self.server.login(self.username, self.password)
